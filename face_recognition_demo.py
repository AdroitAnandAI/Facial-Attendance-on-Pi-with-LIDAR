...
"""
 ...
 extensively on RPi 4B with Intel Movidius NCS2, RPLIDAR A1 M8 with speaker.
...
from lidar_getdist import getObjectDistance
...
class Visualizer:
    ...
    def __init__(self, args):
        ...
        #print("&*******************************************&******************************&")
        self.pplDetected = deque(maxlen=30)
        self.lastFrames = deque(maxlen=30)


        # This is the Publisher
        self.client = mqtt.Client()
        self.client.connect("test.mosquitto.org", 1883, 600)
    ...
    def openDoorAccess(self, identity, name):

        if self.frame_count % 30 == 0:
            # The distance of the object between angle1 and angle 2 is taken
            # The angle depeodns on the direction of LIDAR and where you expect the person to be
            objDistance = getObjectDistance(175, 185)

            print("Person is at distance = " + str(objDistance))

            # This value can be modified to change distance sensitivity
            if (objDistance < 900):

                # if the person is identified then open the door, if person is near
                if 'Unknown' not in identity:
                    # print("*********************PUBLISHING*************************")

                    # Message to register attendance
                    self.client.publish("safetycam/topic/register", identity)
                    self.client.publish("safetycam/topic/blinkt", "green")
                    self.peoplecount = self.peoplecount + 1
                    os.system('espeak \"Hey ' + name + ' Good Day to you. Welcome to the company\"')
                    return True
                else:
                    self.client.publish("safetycam/topic/blinkt", "red")
                    os.system('espeak \"Sorry, your face is not registered. A security personnel would assist you shortly\"')

        return False
    	
    def registerAttendance(self, name):

        log.debug("People Detected = " + str(self.pplDetected))

        if (name not in self.pplDetected):

            doorOpened = self.openDoorAccess(
                                    str(self.peoplecount)+"|"+name+"|"+str(self.date)+"|"+time.strftime('%X')+"|00:00", name)
            if (doorOpened):
                self.pplDetected.extend([name])
    ...
    def draw_detection_roi(self, frame, roi, identity):
        ...
        if label not in self.lastFrames:
            self.pplDetected.clear()

        self.registerAttendance(label)

        self.lastFrames.extend([label])
    ...
    def draw_detections(self, frame, detections):
        ...
        # log.debug("Detection length = " + str(len(detections)))

        if len(detections) == 0:
            self.lastFrames.extend([''])
